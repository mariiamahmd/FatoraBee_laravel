...
public function import(Request $request)
{
    ...
    foreach ($data as $row) {
        Quote::create([
            ...
            'amount'       => $row[3] ?? 0,
            'date'         => $this->safeDate($row[4]) ?? now(),
            'valid_until'  => $this->safeDate($row[5]) ?? now()->addDays(30),
            'items'        => '[]',
        ]);
    }
...
/**
 * Try to parse a date, return null if invalid.
 */
private function safeDate($value)
{
    try {
        return $value && strtotime($value) ? Carbon::parse($value) : null;
    } catch (\Exception $e) {
        return null;
    }
}
